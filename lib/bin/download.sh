...
__asdf_bin() {
  ...
  if kc_asdf_is_ref; then
    url="${ASDF_OVERRIDE_REF_REPO:-https://github.com/google/go-jsonnet.git}"
  ...
  if [[ "$mode" == "custom" ]]; then
    ...
    if kc_asdf_enabled_feature checksum; then
      local checksum_url
      checksum_url="https://github.com/google/go-jsonnet/releases/download/v{version}/checksums.txt"
